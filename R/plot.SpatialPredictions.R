...
plot.SpatialPredictions <- function(x, plot.predictions=TRUE, vertical=FALSE, col=SAGA_pal[[1]], ...){
  ...
  if(sum(!is.na(x@validation$residual))>2){
    ...
    vv <- gstat::variogram(as.formula("observed~1"), x@validation)
    ...
    vvres <- gstat::variogram(as.formula(paste(paste(x@variable, "residual", sep="."), "~ 1")), x@observed[paste(x@variable, "residual", sep=".")]) # model residuals
    if(sum(!is.na(x@validation$residual))>2){
      vve <- gstat::variogram(residual ~ 1, x@validation["residual"]) # validation residuals
      ...
      vline <- gstat::variogramLine(vgmmodel, maxdist=max(vvres$dist), n=length(vvres$dist))
